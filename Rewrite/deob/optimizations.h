...
bool collapse_add_sub(csh handle, List *list, uint8_t mode) {
	...
	while(current) {
		...
		//if(VERBOSE) print_insn("\n[I] The first ADD/SUB instruction is: ", first_insn);
		...
		if(is_memory_insn(first_insn)) {
			...
			uint8_t base = get_base(first_insn);
			uint8_t index = get_index(first_insn);
			uint32_t scale;
			get_scale(first_insn, &scale);
			uint32_t disp;
			get_disp(first_insn, &disp);
			ListEntry *next_use = first;
			Instruction *instruction;
			InsnAccess mem_access = { .mem.base = base, .mem.index = index, .mem.scale = scale, .mem.disp = disp, .op_type = X86_OP_MEM };
			bool is_add = (cmp_id(first_insn->insn->id, X86_INS_ADD)) ? true : false;
			uint64_t imm;
			get_imm(first_insn, &imm);
			//search the next access to the memory location
			while(next_use && next_use->next && (next_use = find_insn_op_general_access(next_use->next, NULL, &mem_access))) {
				instruction = (Instruction *)next_use->content;
				//check if the instruction is ADD/SUB [MEM], IMM
				uint64_t tmp_imm;
				if((cmp_id(instruction->insn->id, X86_INS_ADD) || cmp_id(instruction->insn->id, X86_INS_SUB))) {
					//check if it is using an immediate
					if(get_imm(instruction, &tmp_imm)) {
						//if(VERBOSE) print_insn("[I] ADD/SUB [MEM], IMM: ", instruction);
						//check if base or index are accessed between 'first' & 'next_use' by non-memory instructions, if yes stop the propagation
						InsnAccess reg_access = { .reg = base, .op_type = X86_OP_REG };
						ListEntry *base_use = first;
						while((base_use = find_insn_op_general_access(base_use->next, next_use, &reg_access)) && is_memory_insn(base_use->content)) {}
						//if(VERBOSE && base_use) print_insn("[I] base_use: ", base_use->content);
						reg_access.reg = index;
						ListEntry *index_use = first;
						while((index_use = find_insn_op_general_access(index_use->next, next_use, &reg_access)) && is_memory_insn(index_use->content)) {}
						//if(VERBOSE && index_use) print_insn("[I] index_use: ", index_use->content);
						if(!base_use && !index_use) {
							//next_use can be updated
							...
							if(tmp_imm == 0) {
								...
								//little check to avoid Access Violation
								if(next == next_use) next = list->first;
								...
								//update the instruction, the immediate can be at max an uint32_t
								if(set_imm(instruction, (uint32_t)tmp_imm)) {
									//set the new ID
									set_id(instruction, new_opcode);
									//reassemble the instruction
									Instruction *updated = update_insn_str(handle, instruction, mode);
									//if(VERBOSE) print_insn("[I] Updated instruction: ", updated);
						...
						//ignore the instruction, it is using a register as source
						//if(VERBOSE) print_insn("[I] ADD/SUB [MEM], REG: ", instruction);
					}
				} else {
					//the propagation can stop here
					next_use = NULL;
				}
			}
		} else {
...
/*
	Name: emulate_stack_reg
	Description: this function will emulate the stack register (ESP/RSP) to create
	a single instruction at the code end, of the type "ADD/SUB ESP/RSP, IMM". The
	function will look for instructions like: MOV ESP, [MEM] and LEA ESP, [MEM]
	and will extract the real value for ESP.
*/
bool emulate_stack_reg(csh handle, List *list, Registers *regs, uint8_t mode) {
	bool optimized = false;
	if(!list || !list->first) return optimized;
	//search the next stack_reg_access
	ListEntry *stk_reg_acc = list->first;
	Instruction *instruction;
	InsnAccess stk_acc = { .reg = X86_REG_RSP, .access_type = CS_AC_WRITE, .op_type = X86_OP_REG };
	while(stk_reg_acc && stk_reg_acc->next && (stk_reg_acc = find_insn_op_access(stk_reg_acc->next, NULL, &stk_acc))) {
		//found an instruction assigning a new value to ESP/RSP
		instruction = (Instruction *)stk_reg_acc->content;
		//if(VERBOSE) print_insn("[I] Assign to stack reg: ", instruction);
		//setup registers
		Registers new_regs;
		copy_reg_context(regs, &new_regs);
		uint64_t old_rsp = regs->rsp;
		emulate_code(handle, list->first, stk_reg_acc->next, &new_regs, NULL, mode);
		uint64_t new_rsp = new_regs.rsp;
		//evaluate the delta between old_rsp & new_rsp
		uint64_t delta = (new_rsp > old_rsp) ? new_rsp - old_rsp : old_rsp - new_rsp;
		//check if delta is different from 0
		if(delta != 0) {
			//normalize the delta to 32 bit
			//if(VERBOSE) printf("[I] RSP delta = 0x%llx\n", delta);
			uint8_t reg = get_reg_at(instruction, REG_FIRST);
			char *mnemonic = (new_rsp > old_rsp) ? "add" : "sub";
			char *op_str = calloc(0xFF, sizeof(char));
			sprintf(op_str, "%s, 0x%lx", cs_reg_name(handle, reg), (uint32_t)delta);
			//assemble the new instruction
			Instruction *new_insn = assemble_insn(mnemonic, op_str, instruction->insn->address, mode);
			//if(VERBOSE && new_insn) print_insn("[I] Assembled insn: ", new_insn);
			//replace ListEntry content
			ListChangeEntry(stk_reg_acc, new_insn);
		} else {
			ListRemove(list, stk_reg_acc);
		}
		//propagate the add/sub
		while(collapse_add_sub(handle, list, mode)) {}
		//start again from first instruction, slow but working
		stk_reg_acc = list->first;
		//mark as optimized
		optimized = true;
	}
	return optimized;
...
/*
	Name: delete_junk
	Description: this function will remove useless instructions. More specifically with useless
	we can identify instructions writing to a register or a memory locaziont that will be
	overwritten without being used before; or nop-like instructions (modifying only EFLAGS reg).
*/
bool delete_junk(csh handle, List *list, uint8_t mode) {
	bool optimized = false;
	if(!list || !list->first) return optimized;
	//check all the instructions for usefulness
	ListEntry *current = list->first, *next;
	Instruction *instruction;
	while(current) {
		next = current->next;
		//identify instruction type (MEMORY or REGISTER instruction)
		instruction = (Instruction *)current->content;
		//extract details
		cs_detail *detail = instruction->insn->detail;
		uint8_t op_count = get_op_count(instruction);
		uint8_t regs_write_count = detail->regs_write_count;
		uint8_t regs_read_count = detail->regs_read_count;
		//check if the instruction is using/writing operands, if not delete it
		if(!op_count && !regs_write_count && !regs_read_count) {
			//theoretically this instruction isn't modifying any reg
			optimized = true;
			ListRemove(list, current);
		} else {
			//check if the instruction is a EFLAGS only instruction
			bool eflags_only = true;
			if(!op_count) {
				for(size_t i = 0; i < regs_write_count && eflags_only; i++) {
					if(detail->regs_write[i] != X86_REG_EFLAGS) eflags_only = false; 
				}
				//an instruction writing only EFLAGS is found, check if the next instruction is reading EFLAGS
				next_instruction = current->next->content;
				detail = next_instruction->detail;
				regs_read_count = detail->regs_read_count;
				bool reading_eflags = false;
				for(size_t i = 0; i < regs_read_count && !reading_eflags; i++) {
					if(detail->regs_read[i] == X86_REG_EFLAGS) reading_eflags = true; 
				}
				//if the next instruction is not reading eflags, delete 'current'
				if(!reading_eflags) {
					//theoretically this instruction isn't useful at all
					ListRemove(list, current);
					optimized = true;
				}
			} else {
				//op_count != 0, some register is modified or read
				if(is_dst_reg(instruction)) {
					//the destination is a register, check if it is overwritten before being read again
					InsnAccess reg_access = { .reg = get_reg_at(instruction, REG_FIRST), .access_type = CS_AC_WRITE };
					ListEntry *next_write = find_insn_op_access(current->next, NULL, reg_access);
					reg_access.access_type = CS_AC_READ|CS_AC_WRITE;
					ListEntry *next_rorw = find_insn_op_access(current->next, NULL, reg_access);
					reg_access.access_type = CS_AC_READ;
					ListEntry *next_read = find_insn_op_access(current->next, NULL, reg_access);
					//check which instruction come before
				} else {
					//the destination is a memory location, check if it is overwritten before being read again
					ListEntry *next_write =
					ListEntry *next_read = 
				}
			}
		}
